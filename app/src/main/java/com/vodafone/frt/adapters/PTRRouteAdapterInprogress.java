...
public class PTRRouteAdapterInprogress extends BaseAdapter {
    private final Context context;
    private List<PTRResponseRouteCommonModel> dataSet;
    private SelectionViewAttendanceListener listener;

    public PTRRouteAdapterInprogress(Context ctx) {
        context = ctx;
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        try {
            ...
            if (!dataSet.get(position).getPlanned_start_time().equalsIgnoreCase("null")
                    && !TextUtils.isEmpty(dataSet.get(position).getPlanned_start_time()))
            ...
            if (!actualStart.equals("") && !actualEnd.equals(""))
                holder.actualtiming.setText(actualStart + " To " + actualEnd);

            else if (!actualStart.equals("") )
                holder.actualtiming.setText(actualStart);

            else if (!actualEnd.equals(""))
                holder.actualtiming.setText(actualEnd);
